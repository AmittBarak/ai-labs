...
def create_full_tree(depth: int, max_depth: int) -> Node:
    if depth >= max_depth:
    ...
    node.left = create_full_tree(depth + 1, max_depth)
    if operator != 'NOT':
        node.right = create_full_tree(depth + 1, max_depth)

    return node


def create_grow_tree(depth: int, max_depth: int) -> Node:
    ...
    node.left = create_grow_tree(depth + 1, max_depth)
    if operator != 'NOT':
        node.right = create_grow_tree(depth + 1, max_depth)

    return node


def ramped_half_and_half(population_size: int, max_depth: int) -> List[Node]:
    population = []
    depth_range = range(1, max_depth + 1)

    for depth in depth_range:
        for _ in range(population_size // (2 * len(depth_range))):
            population.append(create_full_tree(0, depth))
            population.append(create_grow_tree(0, depth))
...
def mutate_ga(tree: Node, max_depth: int) -> Node:
    ...
    if node_to_mutate.value in ['A', 'B']:
        ...
        if new_operator == 'NOT':
            ...
            node_to_mutate.right = create_grow_tree(0, 2)
