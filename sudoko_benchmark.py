...
max_generations = [250, 500, 750, 1000, 1250]
crossover_methods = [cycle_crossover_2d]
mutation_methods = [invert_mutation_sudoku]
selection_methods = [SelectionMethod.SUS, SelectionMethod.RWS]
elitism_sizes = [0.1, 0.2, 0.3, 0.4]
aging_options = [True]
...
def run_test(game_index, mutation_rate, population_size, max_gen, crossover_method, mutation_method, selection_method, elitism_size, use_aging) -> float:
    ...
    best_individual, best_fitness, all_fitness_scores, all_generations = run_genetic_algorithm(
        GeneticSettings(
            ...
            adjust_parameters=utils.adjust_parameters
        ),
    )
